...
class CountFeatures extends AlignmentSimilarity {
	...
	override def featurize(sv: SVec, a: Alignment, report: Document, passage: Document) {
		...
		featureIndexer.addStable("diff#PAS", math.abs(rs - ps) / 2d)
		featureIndexer.addStable("sum#PAS", (rs + ps) / 200d)
		featureIndexer.addStable("min#PAS", math.min(rs, ps) / 20d)
		featureIndexer.addStable("max#PAS", math.max(rs, ps) / 100d)

		// density of preds/args
		featureIndexer.addStable("sameDensityPAS", bool2value(math.abs(rd - pd) < 0.2d))
		featureIndexer.addStable("diffDensityPAS", math.abs(rd - pd) * 5d)
