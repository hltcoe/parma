...
class TEDAlignment extends AlignmentSimilarity {
	...
	override def featurize(sv: SVec, a: Alignment, report: Document, passage: Document) {
		...
		featureIndexer.start(sv)
			featureIndexer.addStable("TEDdistance", tdistance, distBinarizer)
			featureIndexer.addStable("TEDheadAlign", headAlign)
			featureIndexer.addStable("TEDbothAlign", alignInBoth)
			featureIndexer.addStable("TEDreportAlign", alignInReport)
			featureIndexer.addStable("TEDpassageAlign", alignInPassage)
