...
class Appositive extends AlignmentSimilarity {
	...
	// if you want this feature to be thread-safe,
	// move this variable to be defined inside of featurize
	...
	override def featurize(sv: SVec, a: Alignment, report: Document, passage: Document)  {
		...
		featureIndexer.addStable("word-jaccard", wordJaccard)
		featureIndexer.addStable("lemma-jaccard", lemmaJaccard)
		featureIndexer.addStable("ner-jaccard", nerJaccard)

		featureIndexer.addStable("unigrams", unigrams)
		featureIndexer.addStable("bigrams", bigrams)
		featureIndexer.addStable("trigrams", trigrams)

		featureIndexer.addStable("head-word", bool2value(headWordMatch))
		featureIndexer.addStable("head-lemma", bool2value(headLemmaMatch))
