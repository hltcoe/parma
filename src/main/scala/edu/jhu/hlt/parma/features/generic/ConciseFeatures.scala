...
class ConciseFeatures extends AlignmentSimilarity with Logging {
	...
	// feature functions
	val ff = new ArrayBuffer[Pipe[(Context, Alignment), DVec]]
	...
	override def featureName(idx: Int): String = {
		...
		if(debug) warning("could not find: idx=%d, ffWidths.sum=%d, ffWidths.size=%d, ffWidths=%s".format(idx, ffWidths.sum, ffWidths.size, ffWidths))
		null
	}
