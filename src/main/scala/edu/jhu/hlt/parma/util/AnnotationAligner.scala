...
object AnnotationAligner {
	...
	val verbose = false
	private val log = Logger.getLogger(AnnotationAligner.getClass.getName)
		...
		for(corefSet <- reportMentions.keySet & passageMentions.keySet) {
			...
			val a = (rHA.mention, pHA.mention) match {
				...
				case (Right(ra), Left(pp)) =>	// translate to predicate?
					...
					null
				case _ =>
					log.warning("encountered a predicate aligned to an argument (corefSet=%s, report=%s, passage=%s), skipping"
