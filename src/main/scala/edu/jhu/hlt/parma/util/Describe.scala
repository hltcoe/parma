...
object Describe {
	...
	def features(sv: SVec, engine: InferenceEngine[_]): String = {
		val sb = new StringBuilder
		for((i, v) <- sv.items) {
	...
	//def linearDecision(features: FeatureVector, parameters: FeatureVector, numRelevant: Int = 15): String = {
	def linearDecision(features: SVec, parameters: DVec, featureNames: Int => String, numRelevant: Int = 15): String = {
		val sb = new StringBuilder
		val dp = VecOps.dot(parameters, features)
		// find the the most relevant features-parameter pairs
		class Index(val idx: Int, val featureWeight: Double, val featureValue: Double) {
			def effectMagnitude: Double = math.abs(effect)
			def effect: Double = featureWeight * featureValue
		}
		val pairs = features.items.map(iv => new Index(iv._1, parameters(iv._1), iv._2)).toBuffer[Index].sortBy(- _.effectMagnitude)
	...
	def docAlignmentInstance(dai: Instance[DocAlignment]): String = {
		val sb = new StringBuilder
		sb.append("eval all:        " + SetBasedEvaluator.generousF1(dai, takePreds=true, takeArgCorefs=true) + "\n")
		sb.append("eval predicates: " + SetBasedEvaluator.generousF1(dai, takePreds=true, takeArgCorefs=false) + "\n")
		sb.append("eval arguments:  " + SetBasedEvaluator.generousF1(dai, takePreds=false, takeArgCorefs=true) + "\n")
		sb.append("<<<<<< Hyp  >>>>>>\n")
	...
	def docAlignment(da: DocAlignment): String =  {
		...
		buf ++= "Predicates:\n"
		val sortedPredAlignments = da.possiblePredicateAlignments.toBuffer.sortBy((pa: PredicateAlignment) => alignment(pa, da.report, da.passage))
		...
		val sortedArgCorefAlignments = da.possibleArgCorefAlignments.toBuffer.sortBy((aca: ArgCorefAlignment) => alignment(aca, da.report, da.passage))
	...
	def argCoref(argc: ArgumentCoref, doc: Document): String = {
		val cm = CanonicalMentionFinder.canonicalMention(argc, doc)
		val headStr = mentionInContext(cm.location, doc)
		val restStrs = argc.filter(_ != cm).map(a => mentionInContext(a.location, doc))
		if(restStrs.size > 0)
	...
	def argument(arg: Argument, doc: Document): String = {
		"(Argument in %s: %s)".format(doc.id, mentionInContext(arg.location, doc))
	}

	
	def predicate(pred: Predicate, doc: Document): String = {
		"(Predicate in %s: %s)".format(doc.id, mentionInContext(pred.location, doc))
	}
	
	
	def alignment(a: Alignment, c: Context): String = alignment(a, c.report, c.passage)
	def alignment(a: Alignment, report: Document, passage: Document): String = a match  {
		case aca: ArgCorefAlignment => "(Alignment\n\treport=%s\n\tpassage=%s\n)".format(
				argCoref(aca.reportCoref, report),
				argCoref(aca.passageCoref, passage))
		case pa: PredicateAlignment => "(Alignment\n\treport=%s\n\tpassage=%s\n)".format(
				predicate(pa.reportPred, report),
				predicate(pa.passagePred, passage))
