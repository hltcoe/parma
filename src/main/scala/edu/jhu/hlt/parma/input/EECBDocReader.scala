...
object EECBConverter {
	def main(args: Array[String]) {
		...
		val das = EECBDocReader.getAlignedDocuments
		println("[EECBConverter] read in %d documents in %.1f seconds, serializing them to %s"
		...
		val commsWithPredArgs = new File("data/eecb/eecb-docs-annotations-concrete.pb")
		...
		println("[EECBConverter] done, converted %d documents in %.1f seconds"
 ...
 * protobuf files.
 */
object EECBDocReader extends DocumentReader[DocAlignment] with Logging {
