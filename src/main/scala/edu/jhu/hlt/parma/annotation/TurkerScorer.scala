...
import edu.jhu.hlt.parma.input.RothFrankDocReader
...
object TurkerScorer {
	...
	object RFAlignments {
		val docAlignments = new RothFrankDocReader(true, true).getAlignedDocuments
		def alignmentMatching(turkerAlignment: DocAlignment): DocAlignment = {
			val try1 = docAlignments.filter(da =>
				da.report.communication.getGuid.getCommunicationId == turkerAlignment.report.id &&
				da.passage.communication.getGuid.getCommunicationId == turkerAlignment.passage.id)
			if(try1.size == 1)
				return try1.head
			else if(try1.size > 1)
				...
				val try2 = docAlignments.filter(da =>
					da.passage.communication.getGuid.getCommunicationId == turkerAlignment.report.id &&
					da.report.communication.getGuid.getCommunicationId == turkerAlignment.passage.id)
				if(try2.size == 1)
					return try2.head
				if(try2.size == 0) {
	...
	private def compareTurkerToGold(turker: DocAlignment, gold: DocAlignment): Double = {
		...
		if(intersection.size == 0 && gold.sureAlignments.size > 0 && turker.sureAlignments.size > 0) {
			...
			for(a <- gold.sureAlignments)
				println("gold: " + Describe.alignment(a, gold.context))
			for(a <- turker.sureAlignments)
				println("turker: " + Describe.alignment(a, turker.context))
		}
