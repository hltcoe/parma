...
object PredArgSelector {
	...
	def identifyPredicatesAndArguments(doc: DocumentBuilder): DocumentBuilder = {
		...
		val mentionsSeen = new HashSet[Mention]
		if(doc.corefs.size > 0) {
			for((coref, corefIdx) <- doc.corefs.zipWithIndex) {
				for(arg <- coref) {
					if(arg.location.width <= MAX_TOKENS) {
						mentionsSeen += arg.location
						newDoc.addArgument(arg)//, corefIdx.toString)
					}
				}
			}
		} else {
			// fill out arguments by looking for Nouns that depend on verbs
			/*
			for((sent, sentIdx) <- doc.sentences.zipWithIndex) {
				for((tok, tokIdx) <- sent.tokens.zipWithIndex) {
					if(sent.isDependentOfVerb(tok)) {
						val mention = MentionBuilder.from(sent, tok)
						mentionsSeen += mention
						newDoc.addArgument(new Argument(mention), mentionsSeen.size.toString)
					}
				}
			}
			*/
			for(sent <- doc.sentences; arg <- potentialArgsByHead(sent)) {
				mentionsSeen += arg.location
				newDoc.addArgument(arg)//, mentionsSeen.size.toString)
			}
		}
		...
		for((sent, sentIdx) <- doc.sentences.zipWithIndex) {
			for((tok, tokIdx) <- sent.tokens.zipWithIndex) {
				if(!sw.contains(tok.getLemma.toLowerCase)
					...
					if(!mentionsSeen.contains(m)) {
						mentionsSeen += m
						newDoc.addPredicate(new Predicate(m))//, "p"+mentionsSeen.size)
					}
				}
			}
		}
