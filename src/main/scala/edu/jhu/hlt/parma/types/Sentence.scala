...
class RichConcreteSent(val concreteSent: edu.jhu.hlt.concrete.Concrete.Sentence, override val index: Int) extends Sentence with Logging {
	...
	override val tokens: IndexedSeq[Token] = {
		...
		def tagHelper(l: java.util.List[TokenTagging], name: String): Buffer[String] = {
			if(l.size == 0) {
				warning("there are no %s tag lists, inserting \"???\"s".format(name))
				cTokens.map(t => "???")
			} else {
				if(l.size > 1) warning("more than one %s tag list, taking the first".format(name))
	...
	override lazy val dependencies: Seq[Dependency[Token]] = {
		...
		if(toks.size > 1)
			warning("more than one tokenization, using the first")
		...
		if(depP.size == 0) {
			warning("there are no dependency parses!")
			Seq()
		}
		else {
			if(depP.filter(_.getMetadata.getTool.toLowerCase.contains("col-ccproc")).size > 1)
				warning("there are more than one col-ccproc dependency parse, choosing: " + depP(0).getMetadata.getTool)
