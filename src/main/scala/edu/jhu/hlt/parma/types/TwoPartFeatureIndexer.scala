...
class TwoPartFeatureIndexer[K](val maxOuterKeys: Int, val maxInnerKeys: Int) extends Serializable {
	...
	/*
	private[this] val outerBits = bits(maxOuterKeys)
	private[this] val innerBits = bits(maxInnerKeys)
	private[this] val outerMask = mask(outerBits, innerBits)
	private[this] val innerMask = mask(innerBits, 0)
	private[this] val outerAlph = new edu.jhu.hlt.parma.util.Alphabet[K]
	*/

	if(innerBits + outerBits > 31) {
		throw new RuntimeException("too many bits to fit in an int! maxOuterKeys=%d maxInnerKeys=%d"
