...
object BadAlignmentDiagnostics {
	...
	def isBad(da: Instance[DocAlignment]) = {
		val cutoff = 0.55
		val badP = SetBasedEvaluator.generousF1(da, takePreds=true, takeArgCorefs=false) < cutoff
		val badA = SetBasedEvaluator.generousF1(da, takePreds=false, takeArgCorefs=true) < cutoff
		badP || badA
	}
